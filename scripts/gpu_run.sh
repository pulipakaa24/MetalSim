...
# call cannot leave a running job with an abandoned-looking lock. The body is a function so bash parses the
# whole file before running it: editing this script no longer breaks instances that are waiting in the queue.
set -u
main() {
