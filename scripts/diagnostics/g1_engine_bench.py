...
usage: python scripts/diagnostics/g1_engine_bench.py N mjwarp DT | N newton ITERS DT   [--rough]"""
...
TERRAIN = "rough" if "--rough" in sys.argv else "flat"
if "--rough" in sys.argv: sys.argv.remove("--rough")
n = int(sys.argv[1]); engine = sys.argv[2]
if engine == "newton":
    it, dt = int(sys.argv[3]), float(sys.argv[4])
    task = G1VelocityTask(n, terrain=TERRAIN, engine="newton", newton_iterations=it, newton_dt=dt); tag = f"{TERRAIN} newton {it} it {dt*1e3:.3g} ms"
else:
    dt = float(sys.argv[3]); task = G1VelocityTask(n, terrain=TERRAIN, physics_dt=dt); tag = f"{TERRAIN} mjwarp {dt*1e3:.3g} ms"
task.reset_all()
...
algo = PPOWarp(task, g1_ppo_config(TERRAIN, 1))
