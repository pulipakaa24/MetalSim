...
the same quantity for the eight terms the reward kernel exposes (tracking lin/ang, feet air time,
feet slide, joint deviation (all groups), flat orientation, action rate, termination), plus the mean
...
TERM_NAMES = ["track_lin_vel_xy_exp", "track_ang_vel_z_exp", "feet_air_time", "feet_slide", "joint_deviation_all", "flat_orientation_l2", "action_rate_l2", "termination_penalty"]
...
def main():
    ...
    task = G1VelocityTask(a.envs, terrain=a.terrain, seed=1, physics_dt=a.physics_dt)
    ...
    ep_sum = np.zeros((n, 8)); ep_len = np.zeros(n, int); done_sums = []; done_lens = []; term_counts = {"fell": 0, "timeout": 0}
    ...
    ret = S.sum(1); print(json.dumps({"ckpt": a.ckpt, "iterations": ck.get("iterations"), "episodes": int(len(L)), "mean_episode_length": float(L.mean()), "mean_return": float(ret.mean()),
                                      ...
                                      "note": "per-second average over the episode like Isaac's RewardManager; joint_deviation_all = hip+arms+fingers+torso; torques/acc/limits/lin_vel_z/ang_vel_xy terms are in the return but not exposed per term"}, indent=1))
