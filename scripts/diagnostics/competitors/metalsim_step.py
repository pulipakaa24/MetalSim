...
R = ROBOTS[name]
import os
if R.get("builder"):                                                  # task model with its own actuators and step
    m = R["builder"]()
...
print("RESULT", json.dumps(dict(engine="metalsim", cone=int(m.opt.cone), robot=name, N=N, mode=mode, nv=m.nv, nu=m.nu, iters=int(sim.m.opt.iterations), ls=int(sim.m.opt.ls_iterations),
      build_s=round(t_build, 1), steps_per_s=round(N * STEPS * SUB / el), substeps=SUB, preset=os.environ.get("G1_PRESET"), finite=ok, base_z_mean=round(qz.mean().item(), 3), overflow=sim.overflow_flags())))
