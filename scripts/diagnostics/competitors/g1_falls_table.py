...
for (ck, p), rs in rows.items():
    falls = np.array([r["falls"] for r in rs])
    ft = {k: sum(r["fall_time_s"][k] for r in rs) for k in ("lt_1", "1_to_5", "5_to_15", "ge_15")}
    ep_t = [f["ep_len_s"] for r in rs for f in r["falls_detail"]]
    med = np.median(ep_t) if ep_t else float("nan")
    dr = {k: sum(r["direction"][k] for r in rs) for k in rs[0]["direction"]}
    cmn = [np.linalg.norm(f["cmd"][:2]) for r in rs for f in r["falls_detail"]]
    cm = np.mean(cmn) if cmn else float("nan")
    lo = sum(r["cmd_norm_at_fall_lt_0.1"] for r in rs)
    per_seed = " / ".join(f"{r['falls']} ({r['episodes']})" for r in rs)
    envs = " / ".join(f"{r['envs_that_fell']} ({r['envs_fell_twice_or_more']})" for r in rs)
    sd = falls.std(ddof=1) if len(rs) > 1 else 0.0
    print(f"| {ck} | {p} | {per_seed} | {falls.mean():.0f} +- {sd:.0f} | {envs} | {ft['lt_1']} / {ft['1_to_5']} / {ft['5_to_15']} / {ft['ge_15']} | {med:.1f} | "
