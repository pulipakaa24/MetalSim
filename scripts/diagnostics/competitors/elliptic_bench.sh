...
if [ "${PROFILE:-1}" = 1 ]; then
  for robot in $MODELS; do
    echo "== profile $robot elliptic"; CONE=elliptic WP_METAL_PROFILE=1 $PY $D/metalsim_profile.py $robot $N kernels 2>&1 | grep -v "^Module\|^Warp\|^   Devices\|^   CUDA\|^   Kernel\|^     \"\|^     /" | head -14
  done
fi
