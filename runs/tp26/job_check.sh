...
echo "--- baseline (installed) $(date +%H:%M:%S)"; MJW_TP_VARIANT='{"label":"installed"}' python scripts/diagnostics/g1_tp_check.py 512 50 --out runs/tp26/check_base.npz 2>&1 | grep -v "^Module\|^Warp\|^   \|^$"
echo "--- worktree (both) $(date +%H:%M:%S)"; PYTHONPATH=$WT MJW_TP_VARIANT='{"label":"worktree regsolve+ldl"}' python -c "import warp, mujoco_warp; print('imports', warp.__file__, mujoco_warp.__file__); import runpy, sys; sys.argv=['g1_tp_check.py','512','50','--ref','runs/tp26/check_base.npz','--out','runs/tp26/check_wt.npz']; runpy.run_path('scripts/diagnostics/g1_tp_check.py', run_name='__main__')" 2>&1 | grep -v "^Module\|^Warp\|^   \|^$"
echo "=== end $(date)"
