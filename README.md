...
| Isaac Lab 3.0-EA G1 tasks (its extra push / mass / reset-velocity events), like-for-like on Isaac's own MuJoCo Warp numerics | flat: +27.5 and +23.9 at iteration 1499 on two runs of the same config and seed that differ only in the training-start defect (the second, with the reset fix: 0 blow-ups, early rise close to Isaac's: +6.5 / +15.4 at 300 / 500 vs +8.4 / +19.6); rough +8.4 / level 5.96 before the actuator-clamp fix. Two trainer defects found and fixed in the process (no reset before the first episode; effort clamp placed where a saturated drive loses implicit damping); final runs with both fixes, three seeds on flat, queued | flat +27.5 (Newton / MuJoCo Warp, 59.4 K on the L4), +28.9 (PhysX, 47.3 K); rough +14.5, level 5.8 (Newton) at iteration 1499, one seed each | [§1.8](docs/PARITY.md#18-isaac-lab-30-ea-reference-isaac-sim-61-measured-on-the-l4-2026-09-25) |
