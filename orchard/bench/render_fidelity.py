...
Usage: python -m orchard.bench.render_fidelity [scene.xml] [camera]
"""
import os, sys
...
def compare(model, cam, qpos=None, res=256, shadows=True, label="", out=None):
    d = mujoco.MjData(model)
    if qpos is not None:
        d.qpos[:len(qpos)] = qpos
    mujoco.mj_forward(model, d)
    ref = mujoco.Renderer(model, res, res); ref.update_scene(d, camera=cam); ri = ref.render()
    rend = Tier0Renderer(model, 1, width=res, height=res, camera=cam, outputs=("rgb",), shadows=shadows)
    ours = rend.render_host([d])["rgb"][0]
    p = psnr(ri, ours); f = flip(ri, ours)
    print(f"{label:30s} shadows={shadows}: PSNR {p:5.2f} dB  FLIP {f:.4f}  mean intensity ref {ri.mean():.1f} ours {ours.mean():.1f}")
    if out:
