...
Robot: `assets/isaac/G1/g1_minimal.usd` (Isaac Lab's asset, converted with `orchard.scene.usd_to_mjcf`),
...
class G1VelocityTask:
    """Isaac Lab velocity task shape for `orchard.learn.ppo_warp.PPOWarp`."""
    ...
    def __init__(self, n, terrain: str = "flat", seed: int = 0, height_scan: bool | None = None, device="metal:0"):
        ...
        if terrain != "flat":
            from orchard.learn.terrain import isaac_rough_terrain
        ...
        self.scanner = None
        if self.use_scan:
            from orchard.learn.terrain import HeightScanner
...
def benchmark_step(task: G1VelocityTask, num_frames: int = 100, warmup: int = 10, capture: bool = True):
    ...
    n = task.n
    from orchard.learn.warp_policy import RolloutBuffers, bump, zero_int
...
def g1_ppo_config(terrain: str, iterations: int, seed: int = 0):
    from orchard.learn.ppo_warp import PPOWarpConfig
...
def train_g1(n=4096, terrain="flat", iterations=1500, seed=0, log_path=None):
    from orchard.learn.ppo_warp import PPOWarp
