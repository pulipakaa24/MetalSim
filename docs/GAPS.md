...
| Deformables on Metal | **closed for cloth, cables and soft volumes** (PARITY §2; fork branch `metalsim-flex`): MuJoCo Warp flex 237/237 on Metal, contact model matched to MuJoCo C (five fixes, one of them a CUDA bug too), 54.1 K env-steps/s at 4096 envs for a cloth + cable scene | remaining: mesh-collider contacts on Metal differ from CPU; the Isaac Lab DeformableObject features listed in §2; the `metalsim-flex` branches are not merged into the forks' `metalsim` branches yet; the flex contact fixes are upstream-worthy (MuJoCo Warp) |
