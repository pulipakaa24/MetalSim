...
| Drift remedies evaluated | measured (fork item 4, CPU): 4 it. / 1.25 ms 14.8 / 51.8 mm (p50 / p99); 4 it. / 0.625 ms 3.2 / 17.7; +4 joint-only passes 5.7 / 26.3; **joint coloring with relaxation 0.8/0.8: 3.4 / 20.1 mm, drop penetration unchanged, no energy gain on 3 seeds** (coloring 1.0 rejected: energy ×1.47 on one seed); joint projection and Featherstone unusable | throughput of the coloring variant at 4096 envs queued; if near the 1.25 ms rate it is the remedy |
...
| Fork solver drive: light, heavily damped hand/arm joints move 5× / 2× faster than MuJoCo C (finger 0.386 vs 0.073 rad at 50 ms); Isaac's policy falls under it | **new, open** (reported to the fork agent) | same fault ActuatorPD had before its gravity-implicit fix on collider-free subtrees; fix the damping against the correct effective inertia; not used in learning runs until then |
