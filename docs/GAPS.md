...
| Contact model: soft contacts and soft joint limits | **measured across 11 settings against the PhysX recordings** (PARITY §1.7 tuning table): hard joint limits fix limit excursions (0.030 → 0.001 rad) at no cost; impact-only stiffening at τ 10 ms brings the hold-impact peak from 3715 to 848 N (Isaac 712) and transfer error to 3.3 cm at 1.02× cost; drop impact peaks stay ~3× Isaac's in every honest setting; air time does not close (0.016–0.022 vs 0.035); elliptic cones ruled out on fidelity | provisional default = impact-only stiffening + hard limits, decided by the pending feet_slide / air-time batch with a seed-noise band; then a like-for-like training run with the chosen preset |
