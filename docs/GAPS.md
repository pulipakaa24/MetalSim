...
| Newton's default relaxation and biased drive | **relaxation fix in progress** in the fork (consistent linear/angular rows read torque 1.001 / gravity 0.999 of analytic; legacy switch keeps the old 1.36/0.78); new defaults being chosen by measurement because consistent linear relaxation ≥ 0.7 injects energy on the G1 (0.6/0.4 stable). Drive fix next. Upstream drafts in `scripts/diagnostics/newton_upstream/` | file the three issues once the fork fixes are validated |
...
| Newton fidelity against PhysX | **measured** (PARITY §1.7): at 4 it. / 0.625 ms Newton is at least as close to PhysX as MuJoCo Warp on every protocol and closer on the chaotic one; at the 1.25 ms training setting it is worse on the 1 m drop (root-height RMSE 0.088 vs 0.035 m). Transfer of Isaac's checkpoints and the fixed-PPO learning run still queued | open, required for the default switch: pick the setting (0.625 ms costs half the throughput, still 2.1× MuJoCo Warp) once the learning run and transfer test land |
