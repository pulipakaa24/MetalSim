...
| Newton fidelity against PhysX | **measured, not met on transfer**: Isaac's PhysX policies over-travel on Newton by 37–48 % at 4 it. / 1.25 ms and 20–35 % at 0.625 ms (MuJoCo Warp within 5 % of Isaac), while the open-loop protocol rows are comparable (PARITY §1.5, §1.7); the drift alters the gait | the fork's drift remedy (joint coloring) and solver drive are the levers; the fork-env learning run and a repeat of the transfer test on the fork decide |
...
| Fork solver drive: light, heavily damped hand/arm joints moved 5× / 2× faster than MuJoCo C | **fixed** (fork 9b0901d3): the damper is now an implicit constraint row solved after the joint's hard rows, with the start-state spring split between an explicit force and the row; zero-g step response vs MuJoCo C at 50 ms: hands 1.03 (was 5.41), arms 1.07 (2.04), legs 1.01 (1.29), every hand joint within 10 %; floating G1 standing vs C at 0.5 s legs 0.006 / arms 0.009 rad (ActuatorPD 0.015 / 0.016); CPU 3σ 0 blown vs 17; pendulum 199.6–200.3 Nm/rad at 1–16 it. Cost: 1-iteration statics worsen (arms 0.034 rad at 1 it., 0.008 at 4) | the fork build currently runs ~30 % slower on the GPU than upstream at default options (145 K → 95–100 K physics-only env-steps/s; measured before this fix, being profiled); not for throughput-sensitive runs until resolved |
