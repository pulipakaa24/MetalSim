...
| Deformables on Metal | **PhysX 5.1 comparison measured** (PARITY §2): XPBD cloth matches PhysX's drape to 2 mm rest height and 3.3 cm height map; flex matches the rope's period (1.006 vs 1.026 s) but not its damping; soft cube too lively (bounce 0.262 vs 0.135 m). Flex branch merged into both forks with rigid results unchanged (Warp f194006, MuJoCo Warp 8fbf965; 7 + 16 patches; `tests/test_deformable.py` skips without the flex build) | **open, being closed**: rope period (XPBD 1.323 vs PhysX 1.026 s: geometry/attachment) and damping (flex log decrement 0.09 vs 0.59) via PBD per-spring damping; flex implicit elasticity damping in the fork (explicit damping unstable above ~1e-4 at 0.5 ms) for ropes and volumes; the soft-cube gap (bounce 0.262 vs 0.135 m, settle 1.97 vs 0.55 s) is a formulation question (PhysX-style co-rotational FEM port, scoped 6–9 days; Newton VBD on Metal, runs but 0.57 K env-steps/s; flex with implicit damping) decided when the Isaac Lab 3.0 recordings show whether PhysX FEM or Newton VBD is the reference; Isaac Lab 3.0 recordings pending; `UPSTREAM.md` fixes not filed |
