...
| Deformables on Metal | **PhysX 5.1 comparison measured** (PARITY §2): XPBD cloth matches PhysX's drape to 2 mm rest height and 3.3 cm height map; flex matches the rope's period (1.006 vs 1.026 s) but not its damping; soft cube too lively (bounce 0.262 vs 0.135 m). Flex branch merged into both forks with rigid results unchanged (Warp f194006, MuJoCo Warp 8fbf965; 7 + 16 patches; `tests/test_deformable.py` skips without the flex build) | **rope closed by fitting** (XPBD with a clamped end, 5 cm segments, a rod-bending constraint and the bending stiffness of PhysX's coarse 11×2×2 mesh measured by cantilever, 12.4× a thin rod's: period 1.077 vs 1.026 s, log decrement 0.63 vs 0.59, first back-swing −0.312 vs −0.305 m, 2.57 M env-steps/s; flex 1.018 s but decrement 0.24). **Implicit flex damping** in the fork (`metalsim-flex` c013067, off by default; backward-Euler CG solve per world in the captured graph; within 5e-5 m of explicit and MuJoCo C where explicit is stable; −20–25 % throughput) makes PhysX-scale damping usable: soft cube bounce 0.132 vs 0.135 m and settle 0.55 vs 0.55 s with a fitted value. **Caveats**: the damping values that match PhysX are 20× (rope) and 40× (cube) its stated parameters, because PhysX's decay is mostly numerical dissipation; the flex cube still sinks 3.6 cm into the ground vs PhysX's 0.16 cm (contact impedance next); volumes' formulation decision waits for the Isaac Lab 3.0 recordings; `UPSTREAM.md` fixes not filed, and the fork's flex/merge commits carry a Co-Authored-By line the MuJoCo Warp project's CLA rejects (`AGENTS.md`), so those would be reworded before filing |
