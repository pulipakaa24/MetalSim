...
|---|---|---|
| Vertical-wall terrain (inverted stairs, boxes): a 0.1 m heightfield turns Isaac's walls into ramps, so policies transfer in neither direction on those cells (7 / 24 and 12 / 24 falls) while pyramid stairs and random rough transfer fine | **new, open** (from the rough like-for-like) | represent stair and box cells as box meshes or a finer heightfield near walls; re-run `scripts/diagnostics/g1_rough_transfer.py` |
...
| Deformables on Metal; upstreaming the forks | in progress (deformables agent, own venv and fork branches); Newton fork upstreamed (issues/PRs filed), Warp and MuJoCo Warp forks not yet | – |
