...
| `tests/test_deformable.py` fails on the main tree ("Cannot synchronize during graph capture" in flex `segmented_sort_pairs`) because the deformables work needs Warp fork commit 9dcb140 from the `metalsim-flex` branch, not yet merged into `metalsim`; 170 other tests pass | **new, open** | merge `metalsim-flex` into `metalsim` on both forks after the rigid-result check, re-export `patches/`, and make the deformable tests skip when the flex build is absent |
