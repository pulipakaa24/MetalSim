...
| Deformables: volume formulation and the 3.0 recordings | **resolved by the 3.0 recordings** (PARITY §2): flex + implicit damping matches PhysX 6's FEM soft cube with PhysX's own parameters (bounce 0.152 vs 0.155 m, settle 0.49 vs 0.50 s), so no FEM port is needed; PhysX's own mesh sweep confirms locking (period 1.016 → 1.477 s from 1 to 8 cells across) and converges to the XPBD `physical` rope; cloth XPBD within 7 mm top height; Newton VBD replay on Metal matches Isaac's Newton recording to < 1 mm (0.3 s CPU check) | remaining: flex continuum-cloth bug (falls through the box), Newton cable pinning (NaN), physx_cloth self-collision, full Metal VBD replay + throughput (queued at low priority) |
