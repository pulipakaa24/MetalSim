...
| Deformables on Metal | **PhysX 5.1 comparison measured** (PARITY §2): XPBD cloth matches PhysX's drape to 2 mm rest height and 3.3 cm height map; flex matches the rope's period (1.006 vs 1.026 s) but not its damping; soft cube too lively (bounce 0.262 vs 0.135 m). Flex branch merged into both forks with rigid results unchanged (Warp f194006, MuJoCo Warp 8fbf965; 7 + 16 patches; `tests/test_deformable.py` skips without the flex build) | remaining: flex damping ceiling (~1e-4 at 0.5 ms) for ropes and volumes; Isaac Lab 3.0 recordings (FEM cloth, Newton VBD) pending; upstream-worthy MuJoCo Warp flex fixes documented in `scripts/diagnostics/deformable/UPSTREAM.md`, not filed |
