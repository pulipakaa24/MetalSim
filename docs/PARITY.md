...
env-steps/s physics-only, 1.21× the recommended preset** (was 1.42×; full PPO loop 1.30× at the task's Newton cap of 10, 39.8 K vs 51.8 K env-steps/s, was 1.44×; but under elliptic cones cap 10 changes states above the cap-100 noise floor (270–546 of 4096 worlds at the cap) and cap 20 is needed, at which the loop premium is **1.60×** (32.3 K), research note §8.5; Go2 623 K, humanoid 666 K, SO-101 847 K
...
|---|---|---|
| rough terrain with everything landed (`runs/tp26/rough2.wrapper.log`) | physics only 63,922-63,969 -> **69,458-69,499**; full env step 45,217-45,268 -> **49,013-49,100 (+8.4 %)**; rollout + inference 51,380-51,420; full PPO loop **44,449-44,504** (first measurement under the preset; the README's rough 41.9 K loop is the default-contact setting) | the same changes; rough capacities (256 / 128) stay explicit (not model-boundable), guarded by `check_overflow()` |
| camera-RL path (Cartpole-RGB 1024 envs, tier 0, 459 K training steps; `runs/tp26/camera.wrapper.log`) | 11,802-12,655 -> 12,931-13,369 env-steps/s incl. training: inside the run-to-run spread; physics is 0.7 % of this step | no render-side change; measurement only |
| MuJoCo Warp fork: per-model unrolled register L'DL factorization of M on Metal (`MJW_METAL_LDL_UNROLLED`) | factor 0.453 -> 0.314 ms per 4096 G1 worlds; physics only 90,618-90,708 -> **93,922-93,985 (+3.6 %)**; full step 60,996-61,248 -> 61,432-62,657 (`runs/tp26/unrolled2.wrapper.log`) | factor and D **bitwise** the serial kernel's on the G1 task, Go2 and the humanoid (`scripts/diagnostics/ldl_lanes_check.py`) |
| **all of the above together** (`runs/tp26/final.wrapper.log`, F1 / F2 vs the day's start R1 / R2 of `runs/mjw_tp/cc.wrapper.log`) | physics only 80,982-81,008 -> **90,395-90,404**; full env step 54,094-54,244 -> **60,775-60,946 (+12.4 %)**; rollout + inference 53,830-54,163 -> **61,397-61,687**; full PPO loop 49,509-50,055 -> **56,351-56,590 (+13.3 %)** | section 5b of the research log; the update (143-159 ms) and learning untouched |
...
| Deformables: Isaac Lab `DeformableObject` (PhysX FEM soft bodies, cloth) | MuJoCo Warp flex on Metal (fork branch `metalsim-flex`, 9 commits; Warp fork `metalsim-flex` 9dcb140 for segmented sort inside graph capture): cloth (grid flex), cables, soft volumes, mesh flex; contact model made to match MuJoCo C (50-contact farthest-point cap, up to 11 contacts per triangle on boxes, capsule cable segments, surface-layer-only volumes, a wrong-element bug for any flex after the first that also affects CUDA); `metalsim/physics/deformable.py` demo (10×10 cloth + 12-vertex cable on a box, 336 DoF, 2 ms) | MuJoCo Warp's flex suite 237/237 on Metal (was 2 failures: constraint-row order); `tests/test_deformable.py` (10 tests: one step from MuJoCo C's state within ~2e-5 m/s on plane, box and cable scenes, 84/84 equal contact sets; cloth landing within 5e-5 m over 100 steps; graph replay) | 54.1 K env-steps/s at 4096 envs (36.5 K at 1024; GPU sorts gave 2–3.6× over Warp's CPU sorts); 20×20 cloth 9.6 K; G1 + cloth + cable 20.0 K at 2048 (4096 out of memory); MetalSim's own XPBD cloth 2.35 M but 5–15 cm vertex error vs MuJoCo C (kept, not default) | **Isaac Lab 3.0 recordings on both backends** (`runs/parity3/isaac/deformable/`, structured tet meshes since 3.0's automatic tet-meshing dependency crashed Kit; measured 2026-09-25): **the locking explanation is confirmed on PhysX's own solver**: the rope's swing period is 1.016 / 1.117 / 1.261 / 1.477 s at 1 / 2 / 4 / 8 cells across (thin-rod value 1.11 s), i.e. the coarse mesh is artificially stiff and the mesh crosses the physical value between 2 and 4 cells; Newton VBD also softens with refinement but at Isaac Lab's Newton default density of 1 kg/m³ its rod moves away from the (then almost rigid) thin-rod value, which points at its 20 solver iterations. Against PhysX 3.0 (CPU device, parameters mapped from PhysX's values, **nothing fitted**): soft cube (4 cells per edge) bounce 0.152 vs 0.155 m, settle 0.49 vs 0.50 s, rest centroid 0.120 vs 0.120 m with flex + implicit damping and PhysX's own parameters; rope 1.113 s / 0.459 m tip drop with the XPBD `physical` preset vs PhysX 1.12 s at 2 cells; cloth top 0.414 vs 0.421 m, mean 0.248 vs 0.272, extent 0.637 vs 0.698, settle 0.78 vs 1.01 s with XPBD (stiffness from Young's modulus × thickness, no bending). So the 20–40× damping fits reported against the 5.1 recording were an artefact of PhysX 5.1's older deformable implementation; PhysX 6's FEM matches with its declared parameters. Newton's own VBD replayed on Metal matches the Isaac Newton recording to under 1 mm over 0.3 s on CPU (full 5 s Metal replay and throughput queued). Defaults per type: cloth XPBD (312 K env-steps/s at 4096 on a 441-vertex cloth), rope XPBD `physical`, soft volume flex + implicit damping, Newton backend VBD on Metal. Open: the flex continuum cloth fell through the box (bug), the Newton cable cannot be pinned (NaN), the physx_cloth port lacks self-collision. Earlier: **first PhysX comparison measured** (Isaac Sim 5.1 recording, `runs/deformable/isaac51/`, bulk measures; PhysX 5.1's particle cloth is position-based springs in 16 substeps, the same family as our XPBD cloth, so its parameters map directly; Isaac Lab 2.3.2 exposes soft volumes but no cloth object): cloth over a box settles at 0.427 m in 1.07 s in PhysX vs **XPBD 0.425 m / 1.14 s, height map within 3.3 cm** (flex 3.8 cm / 2.62 s); hanging rope swing period 1.026 s with log decrement 0.59 vs flex 1.006 s but decrement 0.09 (XPBD chain 1.323 s); soft cube bounces to 0.135 m and settles in 0.55 s vs flex 0.262 m / 1.97 s. Recommended defaults: XPBD for cloth (312 K env-steps/s at 4096), flex for cables and soft volumes (11.7 K / 3.9 K); main gap: flex cannot reach PhysX's damping (its elasticity damping blows up above ~1e-4 at 0.5 ms). Isaac Lab 3.0 recordings (PhysX FEM cloth and Newton VBD) pending; the earlier MuJoCo C comparison establishes implementation correctness only (the reference is PhysX; matching MuJoCo C more exactly is not the same as matching PhysX, cf. the foot-collider result in §1.5); an Isaac-side deformable protocol (cloth drape, cable swing, soft-cube drop with PhysX FEM/cloth and Isaac Lab's material defaults, compared on bulk behaviour) is queued. Also partial: mesh contacts on Metal differ from CPU (3.7e-2 m/s median one-step error; matters for cloth on the G1's mesh colliders); no kinematic node targets, per-element stress, per-world material randomization, tetrahedral-mesh bodies, flex–heightfield collision; Isaac publishes no deformable throughput **Newton VBD on Metal replays Isaac Lab 3.0's Newton recording** (same solver, Metal vs CUDA; `.venv-newton152`, Newton 1.5.2; measured 2026-09-25 evening, `runs/parity3/isaac/deformable/newton_metal.log`, research note §13): cloth 1089 particles within 0.7 mm at 5 s (height on box 0.427 / 0.427 m, extent 0.857 / 0.857 m, settle 4.93 / 4.93 s), cubes 2/4/8 cells within 0.2–0.4 mm (4-cell bounce 0.1110 / 0.1112 m), rods 1/2/4 cells across within 0.1–1.2 mm with identical droop and period; the 8-cell rod (6561 nodes) matches to 1.9 mm at 2 s and diverges to 14 cm at 5 s while still swinging undamped (float32 rounding growth; period 0.658 / 0.658 s). Newton VBD throughput on Metal, 20 iterations × 4 substeps per 5 ms frame, 256 / 1024 / 4096 worlds: cloth 1.54 / 1.43 / 1.21 K env-steps/s, rod (44 nodes) 6.7 / 11.8 / 6.6 K, cube (125 nodes) 2.7 / 0.94 / 0.84 K; at 4096 worlds MetalSim's XPBD cloth (441 vertices) does 0.31 M, flex cube 2.9 K, XPBD rod 2.57 M. Newton VBD is the engine for comparisons against 3.0's Newton backend; MetalSim's defaults stay XPBD (cloth, rope) and flex + implicit damping (soft volumes). |
