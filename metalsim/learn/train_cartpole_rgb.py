...
def main():
    ...
    ap.add_argument("--spp", type=int, default=None, help="tier 2 paths per pixel (default: 4 physical, 1 legacy)"); ap.add_argument("--bounces", type=int, default=2)
    ap.add_argument("--render_mode", default="physical", choices=["physical", "legacy"], help="tier-2 shading (legacy = before 2026-09-25)")
    ...
    env = CartpoleRGBEnv(CartpoleRGBConfig(num_envs=a.envs, tier=a.tier, spp=a.spp, max_bounces=a.bounces, seed=a.seed, render_mode=a.render_mode))
    ...
    def log(msg):
        ...
        if f: f.write(msg + "\n"); f.flush()
    log(f"Cartpole-RGB camera PPO: {a.envs} envs, tier {a.tier} ({a.render_mode if a.tier == 2 else ''} spp {env.rend.spp if a.tier == 2 else '-'}, bounces {a.bounces}), image-only 100x100, Isaac skrl config")
    algo = PPO(env, cfg)
